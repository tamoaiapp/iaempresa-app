...
/**
 * GET /api/zapbot/check-token?token=XXX
 * Validates the token and updates access telemetry (count + IPs).
 * Returns { valid, payment_id?, first_access_at?, access_count? }
 */
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error("[check-token zapbot] supabase err:", error.message);
